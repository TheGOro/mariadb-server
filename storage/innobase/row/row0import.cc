                                  ...
                                  size_t space_id, byte *page_compress_buf)
{
  ...
  const bool should_be_compressed=
      is_full_crc32_compressed || fil_space_t::is_compressed(space_flags);
  ...
  if (should_be_compressed && !page_actually_compressed)
    return DB_CORRUPTION;
                                       ...
                                       const row_import &cfg)
{
  ...
  if (dberr_t err= decrypt_decompress(space_crypt, space_flags,
                                      {page.get(), physical_size}, space_id,
